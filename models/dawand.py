...
from util.losses import arap, count_loss, count_loss_v2, count_loss_l0, gcsmoothness
...
class DAWand:
    ...
    def __init__(self, opt):
        ...
        if self.opt.distortion_loss == "count_v2":
            self.delta = self.opt.delta
            self.distortion_loss_fcn = count_loss_v2
        if self.opt.distortion_loss == "count_l0":
            self.delta = self.opt.delta
            self.distortion_loss_fcn = count_loss_l0

        # GC smoothness loss
        if self.opt.gcsmoothness:
            self.gcsmoothness_w = self.opt.gcsmoothness_weight
        ...
        if self.is_train == True:
            ...
            # Smoothness annealing
            if self.opt.gcsmoothness_anneal:
                self.gcsmoothness_w = 0
    ...
    def optimize_supervised(self, epoch=0):
        # If using count v2 (annealed l0), then anneal delta to max value 1000 (approximates step fcn)
        if self.opt.distortion_loss == "count_v2" and self.opt.deltaschedule:
            self.delta = self.opt.delta * np.log(1 + epoch/self.opt.niter * (np.exp(1) - 1))

        if self.opt.min_delta is not None and self.opt.deltaschedule:
            self.delta = epoch/self.opt.niter * self.opt.min_delta + (1 - epoch/self.opt.niter) * self.opt.delta
        ...
        return self.total_loss.detach().item(), loss_dict, out.detach(), self.labels[i][:len(faces)].detach()
    ...
    def optimize_parameters(self, epoch=0, evalmode=False):
        ...
        if self.opt.distortion_loss == "count_v2":
            if evalmode:
                ...
                self.delta = 1000 * np.log(1 + epoch/self.opt.niter * (np.exp(1) - 1))

        # Delta scheduler
        if self.opt.min_delta is not None:
            if evalmode:
                self.delta = self.opt.min_delta
            else:
                self.delta = epoch/self.opt.niter * self.opt.min_delta + (1 - epoch/self.opt.niter) * self.opt.delta
        ...
        for i in range(len(self.mesh)):
                                ...
                                if self.opt.step1paramloss:
                                    if self.distortion_loss_fcn is not None:
                                        s1_countloss = self.distortion_loss_fcn(distortion_energy_s1, fareas, return_softloss=False, device=self.device,
                                                                            threshold = self.opt.arapthreshold, delta=self.delta, debug=self.opt.debug)
            ...
            if self.opt.gcsmoothness:
                ...
                if self.opt.gcsmoothness_anneal == "linear":
                    self.gcsmoothness_w = epoch/self.opt.niter * self.opt.gcsmoothness_weight

                smoothness_loss = self.gcsmoothness_w * gcsmoothness(mesh_out, mesh)
        ...
        if evalmode==False:
            ...
            # If debugging, then print out the gradient values
            if self.opt.debug:
                grads = []
                for p in self.net.parameters():
                    if p.grad is None:
                ...
                print(f"Gradient quintiles: {torch.quantile(grads, torch.linspace(0,1,5).to(grads.device).float())}")
