...
from util.losses import arap_v2, count_loss, count_loss_v2, gcsmoothness
...
class IntSegModel:
    ...
    def __init__(self, opt):
        ...
        self.ffpost = opt.ffpost
        self.ffalgo = opt.ffalgo
        self.use_net_pred = opt.use_net_pred
        ...
        self.net = networks.define_classifier(opt, self.input_nc, opt.ncf, self.gpu_ids, opt.arch, opt.init_type, opt.init_gain, 
                                              opt.ninput_edges, opt.nclasses)
    ...
    def set_input(self, data):
        ...
        if self.opt.semantic_loss:
            self.anchor_fs_labels = data['anchor_fs_labels']
    ...
    def forward(self, layer=None, export_pool=False):        
        ...
        # Debugging 
        # if self.opt.freeze_net == True or self.is_train == False: 
        #     assert not deep_features.requires_grad
        # else:
        #     assert deep_features.requires_grad
            
        # Sanity check the predicted outputs at this stage
        # import polyscope as ps 
        # mesh = self.mesh[0]
        # vs, fs, _ = mesh.export_soup() 
        # preds = out[0] 
        # anchor_pos = np.mean(vs[fs][mesh.anchor_fs], axis=1)
        # ps.init() 
        # ps_mesh = ps.register_surface_mesh("mesh", vs, fs, edge_width=1)
        # ps_mesh.add_scalar_quantity("preds", preds.detach().squeeze().numpy()[:len(fs)], defined_on='faces', enabled=True)
        # ps_curve = ps.register_curve_network("anchor", anchor_pos, np.array([[0,0]]))
        # ps.show()
    ...
    def optimize_parameters(self, epoch=0, evalmode=False):
        ...
        if self.opt.export_view_freq > 0 and epoch % self.opt.export_view_freq == 0 and epoch != 0 and self.opt.export_pool == True:
            export_pool = True
        ...
        if self.ffalgo == True: 
            ...
            for i in range(len(self.mesh)):
                mesh = self.mesh[i]
                ff_out.append(floodfill_scalar_v2(mesh, out[i], mesh.anchor_fs[0]))
            out = torch.stack(ff_out)
